...
*/
	if(myConnection->nxt_connect() == true){
		...
		for(int i = 0; i < sonarcount; i++){
			myConnection->openSonarOnPort(this->sonars[i]);
		}
		//same for light sensors
		for(int i = 0; i < lightsensorcount; i++){
			myConnection->openLightActiveOnPort(this->lightsensors[i]);
		}
		//same for sound sensors
		for(int i = 0; i < soundsensorcount; i++){
			myConnection->openSoundOnPort(this->soundsensors[i]);
		}
		//same for bumpers
		for(int i = 0; i < bumpercount; i++){
			myConnection->openBumperOnPort(this->bumpers[i]);
		}
  ...
  // return -1, and a NACK will be sent for you, if a response is required.
		...
		if(this->diffdrive && this->rightwheel != -1 && this->leftwheel != -1){
			if(abs(position_cmd.vel.px) <= 100 && abs(position_cmd.vel.pa) <= 100){
				...
				if(a == 0){
					...
					myConnection->nxt_set_motor(x, rightwheel);
					myConnection->nxt_set_motor(x, leftwheel);
				...
				//go left or right
					...
					if(a > 0){
						myConnection->nxt_set_motor(-abs(a), rightwheel);
						myConnection->nxt_set_motor(abs(a), leftwheel);
					}
				...
				//case 5 go foward right
					...
					myConnection->nxt_set_motor(x - (x-a), rightwheel);
					myConnection->nxt_set_motor(x - (x-(a/2)), leftwheel);
				}
				//case 6 go foward left
				else if(x > 0 && a < 0){
					myConnection->nxt_set_motor(x - (x-(a/2)), rightwheel);
					myConnection->nxt_set_motor(x - (x-a), leftwheel);
				}
	...
	if (Message::MatchMessage (hdr, PLAYER_MSGTYPE_REQ, PLAYER_RANGER_REQ_POWER, this->light_addr))
	{
		...
		if(state == 1){
			...
			for(int i = 0; i < lightsensorcount; i++){
				myConnection->openLightPassiveOnPort(this->lightsensors[i]);
			}
		}
	...
	if (Message::MatchMessage (hdr, PLAYER_MSGTYPE_CMD, PLAYER_GRIPPER_CMD_OPEN, this->gripper_addr))
	{
		...
		if(gripperangle > 0){
			myConnection->nxt_set_motor(100, gripperport);
			myConnection->nxt_set_motor_with_limit(100, gripperangle, gripperport);
...
void NXTDriver::UpdateSonars()
{
	...
	for(int i = 0; i < sonarcount; i++){
		ranges[i] = myConnection->getSonarValue(this->sonars[i]);
	}
...
void NXTDriver::UpdateLightSensors()
{
	...
	for(int i = 0; i < lightsensorcount; i++){
		ranges[i] = myConnection->getLightValue(this->lightsensors[i]);
	}
...
void NXTDriver::UpdateSoundSensors()
{
	...
	for(int i = 0; i < soundsensorcount; i++){
		ranges[i] = myConnection->getSoundValue(this->soundsensors[i]);
	}
...
void NXTDriver::UpdateBumpers()
{
	...
	for(int i = 0; i < bumpercount; i++){
		bumpervalues[i] = myConnection->getBumperValue(this->bumpers[i]);
	}
